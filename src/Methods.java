...
public class Methods {
	...
	/**
	 * Procedimiento que muestra por pantalla los diferentes turnos de un combate.
	 * Primero, se crea un vector alternando los jugadores propios y del enemigo.
	 * Segundo, se muestran todos los personajes de ese vector con el procedimiento
	 * printBoard().
	 * Tercero, se recorre ese vector para ir atacando de acuerdo a las propiedades
	 * de cara personaje.
	 * 
	 ...
	 */
	public static int startGame(Scanner in, String[] playerDeck, String[] enemyDeck) {
		...
		// Solo un nextLine es necesario si el buffer ya estaba limpio
	...
	/**
	 * Procedimiento que rellena el vector de juego en función de los vectores
	 * propios y del enemigo
	 * Primero, inicializamos el vector de juego a todas sus posiciones con cadena
	 * vacía "".
	 * Segundo, alternamos los diferentes personajes en el vector: pares->jugador,
	 * impares->enemigo
	 * 
	 * @author Alejandro Ortega
	 * @param playerDeck Referencia a un vector de Strings que contiene los
	...
	/**
	 * Procedimiento que simula limpiar la consola al añadir 10 saltos de línea
	...
	/**
	 * Función que comprueba si hay un ganador, comprobando los personajes vivos en
	 * el vector de juego
	 * 
	 * @author Alejandro Ortega
	 * @param gameDeck Referencia al vector de juego con los personajes intercalados
	 * @param j        Posición en el vector del turno actual: pares->jugador
	 *                 impares->enemigo
	 * @return 1 Ganador Jugador; 0 Ganador Enemigo; -1 No hay ganador todavía
	 */
	 ...
	 * personaje: Valkiria, Goblin &amp; PEKKA
	 * En caso de que no haya personajes, avanza si el valor de advance es
	 * verdadero.
	 * 
	 * @author Alejandro Ortega
	 * @param gameDeck Referencia al vector de juego
	 * @param j        Posición del vector que representa el turno: pares->jugador,
	 *                 impares->enemigo
	 * @param advance  Valor que indica si el personaje debe avanzar si no hay
	 *                 ningún jugador al que golpear
	 */
	public static void valkyrieAttack(String[] gameDeck, int j, boolean advance) {
		...
		for (; i < gameDeck.length; i += 2) {
			if (gameDeck[i].length() > 0) {
				if (turn.equals("Player")) {
					if ((gameDeck[j].charAt(1) - '0' > 0)
							&& (gameDeck[i].charAt(1) - '0') == (gameDeck[j].charAt(1) - '0' - 1)) {
						if (gameDeck[i].charAt(2) == gameDeck[j].charAt(2)) {
							hitted = true;
							if (fightCharacters(gameDeck[j].charAt(0), gameDeck[i].charAt(0))) {
								System.out.println(getCharacterName(gameDeck[j].charAt(0)) + " ["
										+ gameDeck[j].charAt(1) + "][" + gameDeck[j].charAt(2) + "] (" + turn
										+ ") ha matado a " + getCharacterName(gameDeck[i].charAt(0)) + " ["
										+ gameDeck[i].charAt(1) + "][" + gameDeck[i].charAt(2) + "] (" + noTurn + ")");
								gameDeck[i] = "";
							} else {
								System.out.println(getCharacterName(gameDeck[j].charAt(0)) + " ["
										+ gameDeck[j].charAt(1) + "][" + gameDeck[j].charAt(2) + "] (" + turn
										+ ") NO ha matado a " + getCharacterName(gameDeck[i].charAt(0)) + " ["
										+ gameDeck[i].charAt(1) + "][" + gameDeck[i].charAt(2) + "] (" + noTurn + ")");
								front = true;
							}
						}
						if ((gameDeck[i].length() > 0) && (gameDeck[j].charAt(2) - '0' > 0)
								&& (gameDeck[i].charAt(2) - '0') == (gameDeck[j].charAt(2) - '0' - 1)) {
							hitted = true;
							if (fightCharacters(gameDeck[j].charAt(0), gameDeck[i].charAt(0))) {
								System.out.println(getCharacterName(gameDeck[j].charAt(0)) + " ["
										+ gameDeck[j].charAt(1) + "][" + gameDeck[j].charAt(2) + "] (" + turn
						...
						if ((gameDeck[i].length() > 0) && (gameDeck[j].charAt(2) - '0' < (Assets.BOARD_COLUMNS - 1))
								&& (gameDeck[i].charAt(2) - '0') == (gameDeck[j].charAt(2) - '0' + 1)) {
							hitted = true;
							if (fightCharacters(gameDeck[j].charAt(0), gameDeck[i].charAt(0))) {
								System.out.println(getCharacterName(gameDeck[j].charAt(0)) + " ["
						...
						hitted = true;
						if (fightCharacters(gameDeck[j].charAt(0), gameDeck[i].charAt(0))) {
							System.out.println(getCharacterName(gameDeck[j].charAt(0)) + " [" + gameDeck[j].charAt(1)
									+ "][" + gameDeck[j].charAt(2) + "] (" + turn + ") ha matado a "
									+ getCharacterName(gameDeck[i].charAt(0)) + " [" + gameDeck[i].charAt(1) + "]["
									+ gameDeck[i].charAt(2) + "] (" + noTurn + ")");
							gameDeck[i] = "";
						} else {
							System.out.println(getCharacterName(gameDeck[j].charAt(0)) + " [" + gameDeck[j].charAt(1)
									+ "][" + gameDeck[j].charAt(2) + "] (" + turn + ") NO ha matado a "
									+ getCharacterName(gameDeck[i].charAt(0)) + " [" + gameDeck[i].charAt(1) + "]["
									+ gameDeck[i].charAt(2) + "] (" + noTurn + ")");
						}
					}
					...
					if ((gameDeck[j].charAt(1) - '0' < (Assets.BOARD_ROWS - 1))
							&& (gameDeck[i].charAt(1) - '0') == (gameDeck[j].charAt(1) - '0' + 1)) {
						if (gameDeck[i].charAt(2) == gameDeck[j].charAt(2)) {
							hitted = true;
							if (fightCharacters(gameDeck[j].charAt(0), gameDeck[i].charAt(0))) {
		...
		if (!hitted && !front && turn.equals("Player") && (gameDeck[j].charAt(1) - '0' > 0) && advance) {
			System.out.println(getCharacterName(gameDeck[j].charAt(0)) + " [" + gameDeck[j].charAt(1) + "]["
					+ gameDeck[j].charAt(2) + "] (" + turn + ") avanza a [" + (char) (gameDeck[j].charAt(1) - 1) + "]["
					+ gameDeck[j].charAt(2) + "]");
	...
	/**
	 * Procedimiento que realiza un ataque a los personajes de la siguiente fila:
	 * Dragon
	 * En caso de que no haya personajes, avanza una fila.
	 * 
	 * @author Alejandro Ortega
	 * @param gameDeck Referencia al vector de juego
	 * @param j        Posición del vector que representa el turno: pares->jugador,
	...
	public static void dragonAttack(String[] gameDeck, int j) {
		...
		for (; i < gameDeck.length; i += 2) {
			if (gameDeck[i].length() > 0) {
				if (turn.equals("Player") && (gameDeck[j].charAt(1) - '0' > 0)
						&& (gameDeck[i].charAt(1) - '0') == (gameDeck[j].charAt(1) - '0' - 1)) {
					hitted = true;
					if (gameDeck[i].charAt(2) == gameDeck[j].charAt(2)) {
						front = true;
				...
				} else if (turn.equals("Enemy") && (gameDeck[j].charAt(1) - '0' < (Assets.BOARD_ROWS - 1))
						&& (gameDeck[i].charAt(1) - '0') == (gameDeck[j].charAt(1) - '0' + 1)) {
					hitted = true;
					if (gameDeck[i].charAt(2) == gameDeck[j].charAt(2)) {
						front = true;
	...
	/**
	 * Procedimiento que realiza un ataque al personaje más lejano utilizando la
	 * distancia euclídea: Arquera &amp; Princesa
	 * 
	 * @author Alejandro Ortega
	 * @param gameDeck Referencia al vector de juego
	 * @param j        Posición del vector que representa el turno: pares->jugador,
	...
	public static void archerAttack(String[] gameDeck, int j) {
		...
		if (indexDistance != -1) {
			if (fightCharacters(gameDeck[j].charAt(0), gameDeck[indexDistance].charAt(0))) {
				System.out.println(getCharacterName(gameDeck[j].charAt(0)) + " [" + gameDeck[j].charAt(1) + "]["
						+ gameDeck[j].charAt(2) + "] (" + turn + ") ha matado a "
						+ getCharacterName(gameDeck[indexDistance].charAt(0)) + " [" + gameDeck[indexDistance].charAt(1)
						+ "][" + gameDeck[indexDistance].charAt(2) + "] (" + noTurn + ")");
	...
	/**
	 * Función que ejecuta los porcentajes de lucha entre dos personajes
	 * 
	 * @author Alejandro Ortega
	 * @param attackerSymbol Carácter del símbolo que ataca
	 * @param defenderSymbol Carácter del símbolo que defiende
	 * @return V: Atacante ha matado a defensor, F: Atacante NO ha matado a defensor
	 */
	public static boolean fightCharacters(char attackerSymbol, char defenderSymbol) {
		...
		switch (attackerSymbol) {
			...
			case (Assets.PK_SYMBOL):
				...
				break;
			default:
				System.out.println("Símbolo erróneo");
				return false;
		}
	...
	/**
	 * Función que devuelve el nombre de un personaje dado su carácter
	 * 
	 * @author Alejandro Ortega
	 * @param symbol Carácter del personaje
	 * @return Nombre del personaje o "Símbolo erróneo" si no existe
	 */
	...
	/**
	 * Función que devuelve el icono de un personaje dado su carácter
	 * 
	 * @author Alejandro Ortega
	 * @param symbol Carácter del personaje
	 * @return Icono del personaje en formato String o "Símbolo erróneo" si no
	 *         existe
	 */
	...
	/**
	 * Función que devuelve el coste de elixir de un personaje dado su carácter
	 * 
	 * @author Alejandro Ortega
	 * @param symbol Carácter del personaje
	 * @return Coste de elixir o "Símbolo erróneo" si no existe
	 */
